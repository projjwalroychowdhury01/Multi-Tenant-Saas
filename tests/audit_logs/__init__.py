...
class TestAuditLogCreation:
    ...
    def test_user_creation_logged(self, auth_client, org):
        """Creating a new user generates an audit log entry."""
        payload = {
            "email": "newuser@example.com",
            "role": "MEMBER",
        }
        res = auth_client.post(f"/orgs/{org.id}/members/", payload, format="json")
    ...
    def test_api_key_creation_logged(self, auth_client, org):
        """Creating an API key generates an audit log entry."""
        payload = {
            "name": "My Key",
            "env": "test",
        }
        ...
        # Audit log should exist with action related to API key creation
        log = AuditLog.objects.filter(
            org=org,
        ).first()
        assert log is not None
